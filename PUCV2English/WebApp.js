    ...
    return data.map(row => {
        ...
        headers.forEach((h, i) => {
            ...
            if (["disponibilidad", "tipo", "usoIngles", "intl", "nivelIngles", "anioIngreso", "compromiso", "carta", "total"].includes(h.toLowerCase())) {
                ...
                if (h === "Correo Electrónico")
                    obj.correo = val;
                else if (h === "Nombre(s)")
                    obj.nombre = val;
                else if (h === "Apellido(s)")
                    obj.apellidos = val;
                else if (h === "RUT")
                    obj.rut = val;
                else if (h === "Fecha de Postulación")
                    obj.fecha = val;
                else if (h === "Categoría Postulante")
                    obj.categoria = val;
                else if (h === "Sede")
                    obj.sede = val;
...
function obtenerUrlConfirmacionConToken(token, action) {
    ...
    if (!isRealCandidate) {
        ...
        if (hojaC) {
            ...
            if (valuesC.length > 0) {
                ...
                if (idxEmailC === -1)
                    idxEmailC = headersC.indexOf("Correo Electrónico");
 ...
 */
function promoverSiguienteEsperaAPI(nivel) {
    try {
