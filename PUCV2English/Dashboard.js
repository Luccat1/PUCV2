 ...
 */
function calcularEstadisticas(resultadosCompletos, datosOriginales, indicesOriginales) {
    const encabezadosR = resultadosCompletos[0];
    const postulantes = resultadosCompletos.slice(1);
    const getIdx = (name) => encabezadosR.indexOf(name);
    ...
    postulantes.forEach(f => {
        ...
        if (!sCruzados[sede])
            sCruzados[sede] = {};
        if (!sCruzados[sede][cat])
            sCruzados[sede][cat] = { suma: 0, contador: 0 };
...
function formatearDatosDashboard(stats) {
    ...
    const addTable = (title, tableStats) => {
 ...
 */
function crearGraficoSede(sheet, statsSede) {
    const chartData = [["Sede", "Nº Postulantes"]];
    for (const s in statsSede)
        chartData.push([s, statsSede[s].contador]);
 ...
 */
function generarYActualizarDashboard(resultadosCompletos, ss, datosOriginales, indicesOriginales) {
