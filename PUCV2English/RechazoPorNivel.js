 ...
 */
function renderCorreoRechazoPorNivel(vars) {
 ...
 */
function enviarCorreosRechazoPorNivel() {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    const placSheet = ss.getSheetByName(CONFIG.SHEETS.PLACEMENT);
    if (!placSheet)
        return "ERROR: Hoja \"Prueba de Nivel\" no encontrada. Ejecuta primero \"Sincronizar Candidatos a Test\".";
    const data = placSheet.getDataRange().getValues();
    if (data.length < 2)
        return "No hay datos en \"Prueba de Nivel\".";
    ...
    if (recipients.length === 0)
        return "No hay destinatarios pendientes de correo de rechazo.";
